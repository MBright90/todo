...
const domManipulator = (() => {
  ...
  function createTodoTable(toDoList) {
    ...
    homeListTable.appendChild(createTableHeaders('ToDo', 'Details', 'Due Date', ''));
    appendTableData(homeListTable, createTableData(toDoList));
    return homeListTable;
  }

  function createUpcomingTable(upcomingDeadlines, includeDescription, isInteractive) {
    ...
    if (includeDescription) {
      if (isInteractive) {
        deadlinesTable.appendChild(createTableHeaders('ToDo', 'Description', 'Days Until Due', ''));
      } else {
        deadlinesTable.appendChild(createTableHeaders('ToDo', 'Description', 'Days Until Due'));
      }
    } else if (isInteractive) {
  ...
  const createNav = () => {
    ...
    createListedLinks(extraUl, ['Completed', 'Settings', 'Contact Us', 'About']);
